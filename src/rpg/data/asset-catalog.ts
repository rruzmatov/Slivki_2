...
import type { CatalogItem, ItemCategory, Requirement } from "../domain/types";
import { catalogItems } from "./catalog";
import { vehicleFoundationByProductId } from "./transport-foundation";
...
const categoryDefinitions: ReadonlyArray<[ItemCategory | string, string, string, string, number]> = [
...
export const assetCategories: readonly AssetCategory[] = categoryDefinitions.map(([id, assetTypeId, name, description, sortOrder]) => ({
  id,
  assetTypeId,
  name,
  description,
  ...
  allowedCapabilities: assetTypes.find((candidate) => candidate.id === assetTypeId)?.defaultCapabilities ?? [],
  status: "active",
  version: 1
}));

const nativeProducts: readonly Product[] = [
  {
    id: "currency_sum",
...
export const assetProducts: readonly Product[] = [...catalogItems.map(toProduct), ...nativeProducts];
export const shopListings: readonly ShopListing[] = catalogItems.map((item) => toListing(item, toProduct(item)));

export function legacyRequirementsToExpression(requirements: readonly Requirement[] = []): RequirementExpression | undefined {
...
function toProduct(item: CatalogItem): Product {
  const category = assetCategories.find((candidate) => candidate.id === item.category);
  if (!category) {
    throw new Error(`Asset category is not registered: ${item.category}`);
  }

  const inventoryMode = inventoryModeFor(item);
  const assetType = assetTypes.find((candidate) => candidate.id === category.assetTypeId);
  if (!assetType) {
    throw new Error(`Asset type is not registered: ${category.assetTypeId}`);
  }
  const vehicleFoundation = vehicleFoundationByProductId[item.id];
  return {
    id: item.id,
...
function toListing(item: CatalogItem, product: Product): ShopListing {
...
function inventoryModeFor(item: CatalogItem): InventoryMode {
...
function capabilitiesFor(item: CatalogItem): readonly AssetCapability[] {
  const capabilities = new Set<AssetCapability>();
  if (item.assetValue > 0 && item.transport?.canSell !== false) capabilities.add("tradable");
  if (item.transport?.canRepair !== false && item.transport) capabilities.add("repairable");
  if ((item.transport?.maintenanceCost ?? 0) > 0) capabilities.add("maintainable");
  if (item.transport?.upgradeSupport) capabilities.add("upgradeable");
  if (item.transport?.canWork) capabilities.add("work_eligible");
  if (item.transport) capabilities.add("travel_eligible");
  if (item.category === "business") capabilities.add("income_generating");
  if (item.category === "ticket") capabilities.add("consumable");
  if (item.category === "ticket") capabilities.add("expirable");
  return [...capabilities];
}

function unlocksFor(item: CatalogItem): readonly UnlockDefinition[] {
  const unlocks: UnlockDefinition[] = (item.transport?.unlockedJobs ?? []).map((targetId) => ({
    type: "job",
    targetId,
    mode: "while_owned"
  }));
...
function requirementToExpressions(requirement: Requirement): RequirementExpression[] {
  const rules: RequirementExpression[] = [];
  if (requirement.expression) rules.push(requirement.expression);
  if (requirement.level !== undefined) rules.push(predicate("player.level.at_least", { value: requirement.level }, `Требуется уровень ${requirement.level}`));
  if (requirement.familyLevel !== undefined) rules.push(predicate("family.level.at_least", { value: requirement.familyLevel }, `Требуется уровень семьи ${requirement.familyLevel}`));
  if (requirement.balance !== undefined) rules.push(predicate("player.balance.at_least", { value: requirement.balance }, `Требуется баланс ${requirement.balance}`));
  if (requirement.itemId) rules.push(predicate("inventory.owns_product", { productId: requirement.itemId }, `Требуется предмет ${requirement.itemId}`));
  if (requirement.itemCategory) rules.push(predicate("inventory.owns_category", { categoryId: requirement.itemCategory }, `Требуется категория ${requirement.itemCategory}`));
  return rules;
}

function predicate(kind: string, params: Record<string, string | number | boolean>, message: string): RequirementExpression {

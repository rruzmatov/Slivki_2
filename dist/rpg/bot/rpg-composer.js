...
exports.createRpgComposer = exports.createRpgRuntime = void 0;
...
const createRpgRuntime = async (options = {}) => {
    const composer = new telegraf_1.Composer();
    const root = await (0, composition_root_1.createCompositionRoot)({ ...options, startScheduler: true });
    ...
    composer.command("rpg", async (ctx) => {
        ...
        await ctx.reply((0, formatters_1.formatProfile)(player, family), mainKeyboard());
    });
    ...
    composer.command(["family", "familyinfo"], async (ctx) => {
        await replyWithDomainErrors(ctx, async () => {
            ...
            await ctx.reply((0, formatters_1.formatFamily)(family));
        });
    });
    ...
    composer.command("asset", async (ctx) => {
        ...
        await replyWithDomainErrors(ctx, async () => {
            ...
            await ctx.reply((0, formatters_1.formatInventoryEntry)(result.item, result.entry));
        });
    });
    composer.command("giveasset", async (ctx) => {
        ...
        await replyWithDomainErrors(ctx, async () => {
            const result = await services.createInventoryGiftQuote(identityFromContext(ctx), entryId, targetId, nowIso());
            await ctx.reply(`🎁 Передать ${result.item.name} игроку ${result.targetPlayer.firstName}?\nОбъект: ${entryId}`, inventoryGiftKeyboard(result.session.id));
        });
    });
    ...
    composer.command(["catalog", "market"], async (ctx) => {
        ...
        await ctx.reply((0, formatters_1.formatCatalogPage)(items, 0, 25));
    });
    ...
    composer.command("item", async (ctx) => {
        ...
        await replyWithDomainErrors(ctx, async () => {
            ...
            await ctx.reply((0, formatters_1.formatItemCard)(item), buyKeyboard(item.id));
        });
    });
    ...
    composer.command("travel", async (ctx) => {
        ...
        if (!locationId) {
            await ctx.reply(["🌍 Локации", "", ...catalog_1.travelLocations.map((location) => `${location.name}: /travel ${location.id}`)].join("\n"));
            return;
        }
        await replyWithDomainErrors(ctx, async () => {
    ...
    composer.command(["trips", "airport", "passport"], async (ctx) => {
        await ctx.reply(["🌍 Путешествия", "", ...catalog_1.travelLocations.map((location) => `${location.name}: /travel ${location.id}`)].join("\n"));
    });
    composer.command("ticket", (ctx) => showCategory(ctx, services, "ticket"));
    composer.command(["worldmap", "map"], async (ctx) => {
        await ctx.reply(["🗺 Карта мира", "", ...catalog_1.travelLocations.map((location) => `${location.name}: /travel ${location.id}`)].join("\n"));
    });
    ...
    composer.command("recoveritem", async (ctx) => {
        await replyWithDomainErrors(ctx, async () => {
            ...
            if (!entryId || !reason)
                throw new Error("Формат: /recoveritem <instanceId> <причина>");
    ...
    composer.action(/^shop_buy_(cash|bank):(.+)$/, async (ctx) => {
        await ctx.answerCbQuery();
        await replyWithDomainErrors(ctx, async () => {
            const paymentSource = ctx.match[1];
            const checkoutId = ctx.match[2];
            const result = await services.confirmPurchase(identityFromContext(ctx), checkoutId, paymentSource, callbackRequestId(ctx), nowIso());
    ...
    composer.action(/^shop_sale_confirm:(.+)$/, async (ctx) => {
        await ctx.answerCbQuery();
        await replyWithDomainErrors(ctx, async () => {
            const result = await services.confirmSale(identityFromContext(ctx), ctx.match[1], callbackRequestId(ctx), nowIso());
    ...
    composer.action(/^iv_gift_confirm:(.+)$/, async (ctx) => {
        await ctx.answerCbQuery();
        await replyWithDomainErrors(ctx, async () => {
            const result = await services.confirmInventoryGift(identityFromContext(ctx), ctx.match[1], callbackRequestId(ctx), nowIso());
...
exports.createRpgRuntime = createRpgRuntime;
const createRpgComposer = async (options = {}) => (await (0, exports.createRpgRuntime)(options)).composer;
exports.createRpgComposer = createRpgComposer;
const mainKeyboard = () => {
    if (!RPG_UI_DISCOVERY_ENABLED)
        return undefined;
    return telegraf_1.Markup.inlineKeyboard([
        [telegraf_1.Markup.button.callback("👤 Профиль", "rpg_profile"), telegraf_1.Markup.button.callback("🏦 Банк", "rpg_bank")],
        [telegraf_1.Markup.button.callback("💼 Работа", "rpg_jobs"), telegraf_1.Markup.button.callback("🛒 Магазин", "rpg_shop")],
        [telegraf_1.Markup.button.callback("🚗 Транспорт", "rpg_transport"), telegraf_1.Markup.button.callback("🏢 Бизнес", "rpg_business")],
        [telegraf_1.Markup.button.callback("🌍 Карта", "rpg_worldmap"), telegraf_1.Markup.button.callback("🏆 Forbes", "rpg_forbes")]
    ]);
};
const buyKeyboard = (itemId) => telegraf_1.Markup.inlineKeyboard([
    [callbackButton("Купить", `shop_quote:${itemId}`), callbackButton("Магазин", "rpg_shop")]
]);
const purchaseKeyboard = (checkoutId) => telegraf_1.Markup.inlineKeyboard([
...
const saleKeyboard = (checkoutId) => telegraf_1.Markup.inlineKeyboard([
    [callbackButton("Подтвердить продажу", `shop_sale_confirm:${checkoutId}`)],
    [callbackButton("Отмена", `shop_cancel:${checkoutId}`)]
]);
const inventoryGiftKeyboard = (sessionId) => telegraf_1.Markup.inlineKeyboard([
    [callbackButton("Передать", `iv_gift_confirm:${sessionId}`)],
    [callbackButton("Отмена", `iv_gift_cancel:${sessionId}`)]
]);
const shopHomeKeyboard = (services) => telegraf_1.Markup.inlineKeyboard(services.listAssetTypes().map((assetType) => [callbackButton(assetType.name, `shop_asset:${assetType.id}`)]));
const categoryKeyboard = (categories) => telegraf_1.Markup.inlineKeyboard([
    ...categories.map((category) => [callbackButton(category.name, `shop_category:${category.id}:0`)]),
    [callbackButton("Назад", "rpg_shop")]
]);
const shopCategoryKeyboard = (items, category, page, pageSize) => {
    const pageItems = items.slice(page * pageSize, (page + 1) * pageSize);
    const rows = pageItems.map((item) => [callbackButton(item.name, `shop_product:${item.id}`)]);
    const navigation = [];
    if (page > 0)
        navigation.push(callbackButton("Назад", `shop_category:${category}:${page - 1}`));
    if ((page + 1) * pageSize < items.length)
        navigation.push(callbackButton("Далее", `shop_category:${category}:${page + 1}`));
    if (navigation.length > 0)
        rows.push(navigation);
    rows.push([callbackButton("Категории", "rpg_shop")]);
    return telegraf_1.Markup.inlineKeyboard(rows);
};
const callbackButton = (text, data) => {
    if (Buffer.byteLength(data, "utf8") > 64)
        throw new Error(`Callback data exceeds Telegram limit: ${data}`);
...
const formatPurchaseQuote = (item, total, expiresAt) => [
    (0, formatters_1.formatItemCard)(item),
    "",
...
const formatSaleQuote = (itemName, total, expiresAt) => [
    `Продажа: ${itemName}`,
...
const replyWithDomainErrors = async (ctx, handler) => {
    try {
        await handler();
    }
    catch (error) {
...
const identityFromId = (id) => ({
    id,
    firstName: `Игрок ${id}`
});
const showCategory = async (ctx, services, category) => {
    await showShopCategory(ctx, services, category, 0);
};
const showShopCategory = async (ctx, services, category, page) => {
    const pageSize = game_balance_1.GAME_BALANCE.pagination.pageSize;
    if (!Number.isSafeInteger(page) || page < 0)
        throw new Error("Некорректная страница магазина");
    ...
    await ctx.reply((0, formatters_1.formatCatalogPage)(items, page, pageSize), shopCategoryKeyboard(allItems, category, page, pageSize));
};
const callbackRequestId = (ctx) => {
    if (!ctx.callbackQuery)
        throw new Error("Не удалось определить callback-запрос");
    return ctx.callbackQuery.id;
};
const showInventoryPage = async (ctx, services, page) => {
    if (!Number.isSafeInteger(page) || page < 0)
        throw new Error("Некорректная страница инвентаря");
    ...
    const lines = pageEntries.map(({ item, entry }) => (0, formatters_1.formatInventoryEntry)(item, entry));
    const navigation = [];
    if (page > 0)
        navigation.push(callbackButton("Назад", `rpg_inventory:${page - 1}`));
    if ((page + 1) * pageSize < entries.length)
        navigation.push(callbackButton("Далее", `rpg_inventory:${page + 1}`));
    await ctx.reply([`🎒 Инвентарь • страница ${page + 1}`, "", ...(lines.length > 0 ? lines : ["Пусто"])].join("\n"), navigation.length > 0 ? telegraf_1.Markup.inlineKeyboard([navigation]) : undefined);
};
const formatInventoryHistory = (events) => {
    const labels = {
